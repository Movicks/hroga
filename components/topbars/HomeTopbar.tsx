...
export default function HomeTopbar() {
  ...
  return (
    <header className="bg-transparent fixed top-0 z-100 px-4 pt-3 lg:px-[5rem] xl:px-[12.7rem] w-full">
      <div className="container bg-transparent flex items-center justify-between gap-6">
        {/* Logo */}
        <Link href="/" className="text-2xl font-bold text-secondary">
          <Image src="/images/HROGA_LOGO2.png" className='w-[3.5rem] max-h-[3.7rem]' alt="HROGA" width={100} height={100} loading="eager" />
        </Link>

        <section className='hidden md:flex gap-15 xl:gap-4 xl:w-[55%] pl-8 pr-4 justify-between bg-[#f3f9d2] h-full py-2 rounded-full overflow-hidden shadow-md'>
          {/* Navigation Links */}
          <nav className="hidden md:flex items-center gap-6">
            {navLinks.map((item, index) => (
              <Link key={index} href={item.href} className="text-gray-700 hover:text-primary transition-colors">
                {item.name}
              </Link>
            ))}
          </nav>
      ...
      >
        <aside
          className={`h-full w-[80%] max-w-xs bg-white px-6 py-8 shadow-xl transition-transform duration-300 ${
